...
void writemem(int addr, int value)
{
  ...
  */
  ...
  /*hack*/
  // The CPU would like to continue but if it has updated the lcd we need to return
  if (have_lcdframe) {
    have_lcdframe = 0;
    return;
  }
...
void run_cpu()
{
  ...
  for(;;) {
    ...
    if(mcy>=spd) {
      ...
      if(now.tv_sec>epoch.tv_sec || (now.tv_sec == epoch.tv_sec && now.tv_usec >= epoch.tv_usec)) {
        t.tv_usec = 0;
        t.tv_sec = 0;
      } 
